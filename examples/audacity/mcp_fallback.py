...
Writes out/fallback_report.json and out/e2e_som.png.
"""
import asyncio
import base64
import json
...
async def main():
    ...
    async with Client(params, elicitation_callback=on_elicit, extensions=[TasksClientExtension()]) as c:
        ...
        await call(c, s, "explain", action="get_track_count")
        await call(c, s, "generate_tone", frequency=660)          # no backend forced
        await call(c, s, "get_track_count")
        await call(c, s, "undo")
        await call(c, s, "get_track_count")
        (OUT / "fallback_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=1, default=str), encoding="utf-8")
        print("REPORT WRITTEN", flush=True)
        return  # fallback scenario ends here
        ...
        (OUT / "fallback_som.png").write_bytes(base64.b64decode(img.data))
